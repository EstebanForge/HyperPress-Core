...
### Fixed
- **Template names surfaced with a leading separator on Windows.** `RecursiveDirectoryIterator` appends backslash separators there even when the base directory was passed with forward slashes, so `substr($pathname, strlen($base_dir))` could leave a leading separator and the `hyperpress/list-endpoints` inventory returned names like `/sub/file`. The relative remainder is now trimmed on both separators before the name is built.
- **Library-mode asset URL detection returned empty on Windows.** `getLibraryModeUrl()` compared raw `__DIR__`-derived paths against `WP_CONTENT_DIR` with literal forward-slash `strpos` checks and `/wp-content/` regexes; on Windows the backslash-separated input failed every branch and the method silently returned `''`, breaking enqueued asset URLs in library mode. Both inputs are now normalized through `wp_normalize_path()` (with a guarded fallback, matching the suite idiom in HyperFields' `LibraryBootstrap`) before the `rtrim` and all branch checks; output URLs are built from the normalized relative path. Same separator-mismatch class as the HyperBlocks containment fix; found in peer review of that change.
